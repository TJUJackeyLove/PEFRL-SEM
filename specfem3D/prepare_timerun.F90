  ...
  ! ZN I do not use if (USE_PEFRL) call prepare_timerun_PEFRL()
  ! ZN in order to avoid the error of using unallocated arrays later on in the code,
  ! ZN since R_**_PEFRL are arguments in subroutine compute_forces_viscoelastic
  call prepare_timerun_PEFRL()
  ...
  if (myrank == 0) then
    ...
    write(IMAIN,*) '************'
    if (USE_PEFRL) then
      write(IMAIN,*) '              scheme:         PEFRL with',NSTAGE_TIME_SCHEME,'stages'
    else
...
! precomputes constants for time integration
  ...
  ! time scheme
  if (.not. USE_PEFRL) then
    ! Newmark time scheme, only single update needed
    NSTAGE_TIME_SCHEME = 1
  else
    ! PEFRL time scheme with 6-stages
    NSTAGE_TIME_SCHEME = 4
  endif
...
!
  ...
  subroutine prepare_timerun_PEFRL()
  ...
  if (USE_PEFRL) then
    NGLOB_AB_PEFRL = NGLOB_AB
    NSPEC_ATTENUATION_AB_PEFRL = NSPEC_ATTENUATION_AB
  else
    NGLOB_AB_PEFRL = 1
    NSPEC_ATTENUATION_AB_PEFRL = 1
  endif

  ! user output
  if (USE_PEFRL) then
    if (myrank == 0) then
      write(IMAIN,*) "preparing PEFRL"
      call flush_IMAIN()
    endif
  endif

  if (ACOUSTIC_SIMULATION) then
    allocate(potential_acoustic_PEFRL(NGLOB_AB_PEFRL),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 2120')
    if (ier /= 0) stop 'Error allocating array potential_acoustic_PEFRL'
    allocate(potential_dot_acoustic_PEFRL(NGLOB_AB_PEFRL),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 2121')
    if (ier /= 0) stop 'Error allocating array potential_dot_acoustic_PEFRL'

    potential_acoustic_PEFRL(:) = 0._CUSTOM_REAL
    potential_dot_acoustic_PEFRL(:) = 0._CUSTOM_REAL
    if (FIX_UNDERFLOW_PROBLEM) then
      potential_acoustic_PEFRL(:) = VERYSMALLVAL
      potential_dot_acoustic_PEFRL(:) = VERYSMALLVAL
    endif
  endif

  if (ELASTIC_SIMULATION) then
    allocate(displ_PEFRL(NDIM,NGLOB_AB_PEFRL),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 2122')
    if (ier /= 0) stop 'Error allocating array displ_PEFRL'
    allocate(veloc_PEFRL(NDIM,NGLOB_AB_PEFRL),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 2123')
    if (ier /= 0) stop 'Error allocating array veloc_PEFRL'

    displ_PEFRL(:,:) = 0._CUSTOM_REAL
    veloc_PEFRL(:,:) = 0._CUSTOM_REAL
    if (FIX_UNDERFLOW_PROBLEM) then
      displ_PEFRL(:,:) = VERYSMALLVAL
      veloc_PEFRL(:,:) = VERYSMALLVAL
    endif

    ! note: currently, they need to be defined, as they are used in some subroutine arguments
    allocate(R_xx_PEFRL(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB_PEFRL),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 2124')
    allocate(R_yy_PEFRL(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB_PEFRL),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 2125')
    allocate(R_xy_PEFRL(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB_PEFRL),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 2126')
    allocate(R_xz_PEFRL(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB_PEFRL),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 2127')
    allocate(R_yz_PEFRL(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB_PEFRL),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 2128')
    if (ier /= 0) stop 'Error allocating array R_**_PEFRL etc.'

    allocate(R_trace_PEFRL(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB_PEFRL),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 2129')
    if (ier /= 0) stop 'Error allocating array R_trace_PEFRL etc.'

    if (SIMULATION_TYPE == 3) then
      allocate(b_R_xx_PEFRL(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB_PEFRL),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 2130')
      allocate(b_R_yy_PEFRL(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB_PEFRL),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 2131')
      allocate(b_R_xy_PEFRL(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB_PEFRL),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 2132')
      allocate(b_R_xz_PEFRL(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB_PEFRL),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 2133')
      allocate(b_R_yz_PEFRL(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB_PEFRL),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 2134')
      if (ier /= 0) stop 'Error allocating array R_**_PEFRL etc.'

      allocate(b_R_trace_PEFRL(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB_PEFRL),stat=ier)
    ...
    ! initializes
    if (ATTENUATION) then
      R_xx_PEFRL(:,:,:,:,:) = 0._CUSTOM_REAL
      R_yy_PEFRL(:,:,:,:,:) = 0._CUSTOM_REAL
      R_xy_PEFRL(:,:,:,:,:) = 0._CUSTOM_REAL
      R_xz_PEFRL(:,:,:,:,:) = 0._CUSTOM_REAL
      R_yz_PEFRL(:,:,:,:,:) = 0._CUSTOM_REAL
      R_trace_PEFRL(:,:,:,:,:) = 0._CUSTOM_REAL
      if (FIX_UNDERFLOW_PROBLEM) then
        R_xx_PEFRL(:,:,:,:,:) = VERYSMALLVAL
        R_yy_PEFRL(:,:,:,:,:) = VERYSMALLVAL
        R_xy_PEFRL(:,:,:,:,:) = VERYSMALLVAL
        R_xz_PEFRL(:,:,:,:,:) = VERYSMALLVAL
        R_yz_PEFRL(:,:,:,:,:) = VERYSMALLVAL
        R_trace_PEFRL(:,:,:,:,:) = VERYSMALLVAL
      endif

      if (SIMULATION_TYPE == 3) then
        b_R_xx_PEFRL(:,:,:,:,:) = 0._CUSTOM_REAL
        b_R_yy_PEFRL(:,:,:,:,:) = 0._CUSTOM_REAL
        b_R_xy_PEFRL(:,:,:,:,:) = 0._CUSTOM_REAL
        b_R_xz_PEFRL(:,:,:,:,:) = 0._CUSTOM_REAL
        b_R_yz_PEFRL(:,:,:,:,:) = 0._CUSTOM_REAL
        b_R_trace_PEFRL(:,:,:,:,:) = 0._CUSTOM_REAL
        if (FIX_UNDERFLOW_PROBLEM) then
          b_R_xx_PEFRL(:,:,:,:,:) = VERYSMALLVAL
          b_R_yy_PEFRL(:,:,:,:,:) = VERYSMALLVAL
          b_R_xy_PEFRL(:,:,:,:,:) = VERYSMALLVAL
          b_R_xz_PEFRL(:,:,:,:,:) = VERYSMALLVAL
          b_R_yz_PEFRL(:,:,:,:,:) = VERYSMALLVAL
          b_R_trace_PEFRL(:,:,:,:,:) = VERYSMALLVAL
        endif
      endif
    endif
  endif
  ...
  if (POROELASTIC_SIMULATION) then
    if (USE_PEFRL) &
      stop 'PEFRL has not been implemented for POROELASTIC_SIMULATION'
  endif

  ! synchonizes
  call synchronize_all()

  end subroutine prepare_timerun_PEFRL
